...
//Date of last editing: Saturday 17th of October 2012
...
MatrixXf fnvalspapi_2D(VectorXf X, MatrixXf Y, VectorXf X_target){
  //evaluate Y_target for X_target given X and Y
    int M = Y.rows(); //cout << M << endl;
	int N =  X_target.size();     //cout <<N << endl;
	MatrixXf rr(M,N); //cout << Y.row(1).size() << endl;
    
	for (int i=0; i< M; i++){  
      rr.row(i)  = fnvalspapi(X,Y.row(i),X_target);  }
	return(rr);
}
...
float rttemp1( VectorXf t_reg,  MatrixXf surfci, MatrixXf surfck,int nknots, float lambda, VectorXf initial){
  //Calculate the cost of the given warping
  // t_reg  : time grid of y_reg
  // surfci : query curve
  // surfck : reference curve
  ...
  return ( (fnvalspapi_2D(t_reg,surfci,hik)-fnvalspapi_2D(t_reg,surfck ,t_reg)).array().pow(2).sum() + D* lambda * (hik - t_reg).array().pow(2).sum() );
}
...
rthink_Output rthik_SA(VectorXf t_reg,  MatrixXf surfci, MatrixXf surfck,int nknots, float lambda){
  ...
  float NewSol; 
  OldSol =  rttemp1(t_reg, surfci, surfck, nknots , lambda, xk);  //Cost of initial solution
  int z= surfci.rows()*129*nknots;                                 //Number of random search to do (proportional to the # of knots) #Originally this was 99
  ...
  while((OldSol > .0001) && (k<z)) {
    ...
    NewSol = rttemp1(t_reg, surfci, surfck, nknots, lambda, xn);  //Cost of new solution
...
//Use with syntax :  [hik(indd,:),ftemp]=rthik_E_2D( surfci.coefs,surfck.coefs,t_reg,nknots,lambda); 
#define IS_REAL_1D_FULL_DOUBLE(P) (!mxIsComplex(P) && mxGetNumberOfDimensions(P) == 2 && !mxIsSparse(P) && mxIsDouble(P))

void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) 
{    
    #define p_surfci	prhs[0]  
	#define p_surfck	prhs[1]  
    ...
    else if(!IS_REAL_1D_FULL_DOUBLE(p_surfci))   //  Check lw
     mexErrMsgTxt("surfci needs be a real 2D full double array."); 
    else if(!IS_REAL_1D_FULL_DOUBLE(p_surfck))   //  Check lw
     mexErrMsgTxt("surfck needs be a real 2D full double array."); 
   ...
   const int NumberOfRows = mxGetM(p_surfci); 
   
   //Read in the variables
   Map<MatrixXd> SurfI(mxGetPr(p_surfci) ,NumberOfRows, LengthOfVector); 
   Map<MatrixXd> SurfK(mxGetPr(p_surfck) ,NumberOfRows, LengthOfVector); 
   ...
   rthink_Output Res = rthik_SA(T_Reg.cast<float>(),SurfI.cast<float>(),SurfK.cast<float>(), knots, float(lamdba));
